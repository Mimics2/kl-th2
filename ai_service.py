...
# ========== TARIFF SYSTEM WITH 8 MARCH DISCOUNT ==========
class Tariff(Enum):
    MINI = "mini"
    STANDARD = "standard"
    VIP = "vip"
...
# 8 марта скидка 20%
MARCH_8_DISCOUNT = 0.2  # 20% скидка
MARCH_8_ACTIVE = True  # Флаг активации скидки

def apply_march_8_discount(price):
    """Применяет скидку 8 марта к цене"""
    if MARCH_8_ACTIVE and price > 0:
        discounted = price * (1 - MARCH_8_DISCOUNT)
...
TARIFFS = {
    ...
    Tariff.STANDARD.value: {
        "name": "⭐ Standard",
        "price": apply_march_8_discount(4),  # Было 4, стало 3.20
        "original_price": 4,
        "currency": "USD",
        "channels_limit": 2,
        "daily_posts_limit": 6,
    ...
    Tariff.VIP.value: {
        "name": "👑 VIP",
        "price": apply_march_8_discount(7),  # Было 7, стало 5.60
        "original_price": 7,
        "currency": "USD",
        "channels_limit": 3,
...
MARCH_8_INFO = {
    "active": MARCH_8_ACTIVE,
    "discount_percent": int(MARCH_8_DISCOUNT * 100),
    "valid_until": "8 марта 2024",
    "message": "🌷 С 8 марта! Действует скидка 20% на все тарифы! 🌷"
}

# ========== PROMPT TEMPLATES WITH 8 MARCH STYLE ==========
...
ДОПОЛНИТЕЛЬНО:
- Текущая дата: {current_date} - наступила весна, приближается 8 марта
- Можно использовать весенние и праздничные мотивы, если это уместно
...
ДОПОЛНИТЕЛЬНО:
- Учитывай тренды {current_date} - весна, приближается 8 марта
- Можно предлагать идеи, связанные с весной, цветами, подарками, женскими темами
