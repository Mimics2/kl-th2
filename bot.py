...
TRIAL_CHANNEL_URL = os.getenv("TRIAL_CHANNEL_URL", "https://t.me/test_channel")  # Ссылка на канал для пробных дней
RAILWAY_DEPLOY_URL = os.getenv("RAILWAY_DEPLOY_URL", "https://railway.app")  # Ссылка на Railway для поддержки
...
KEY_ROTATION_ENABLED = True  # Включение ротации ключей
...
class Tariff(Enum):
    ...
    VIP = "vip"
    ADMIN = "admin"
    STANDARD_TRIAL = "standard_trial"  # Пробный тариф
...
# ========== ADVANCED AI SESSION MANAGER WITH KEY ROTATION ==========
class AdvancedAISessionManager:
    ...
    def rotate_key(self, user_id: int) -> Tuple[Optional[str], int]:
        """Ротация ключа при ошибке"""
        session = self.get_session(user_id)
        
        if not self.key_rotation_enabled:
            # Если ротация отключена, возвращаем текущий ключ
            current_index = session['current_key_index'] % len(GEMINI_API_KEYS)
            return GEMINI_API_KEYS[current_index], current_index
        
        # Ищем следующий доступный ключ
        start_index = (session['current_key_index'] + 1) % len(GEMINI_API_KEYS)
        for i in range(len(GEMINI_API_KEYS)):
            current_index = (start_index + i) % len(GEMINI_API_KEYS)
            key = GEMINI_API_KEYS[current_index]
            
            if self._is_key_available(key) and key not in session.get('failed_keys', set()):
                session['current_key_index'] = current_index
                session['current_attempts'] = 0
                return key, current_index
        
        # Если все ключи недоступны, возвращаем первый
    ...
    def get_best_key(self, user_id: int) -> Tuple[Optional[str], int, str]:
        ...
        # Если есть последний успешный ключ и он доступен
        ...
        current_key = GEMINI_API_KEYS[session['current_key_index'] % len(GEMINI_API_KEYS)]
        if self._is_key_available(current_key) and current_key not in session.get('failed_keys', set()):
            return current_key, session['current_key_index'] % len(GEMINI_API_KEYS), self.get_current_model()
        
        # Если текущий ключ недоступен, ищем следующий
        return self.rotate_key(user_id) + (self.get_current_model(),)
    ...
    def mark_key_success(self, key: str, user_id: int):
        ...
        # Сбрасываем статистику ошибок для всех ключей через некоторое время
        for k in list(session.get('failed_keys', set())):
            key_stats = self.key_stats.get(k)
            if key_stats and key_stats.get('last_success'):
                time_since_last_success = datetime.now(MOSCOW_TZ) - key_stats['last_success']
                if time_since_last_success.total_seconds() > 3600:  # 1 час
                    session['failed_keys'].discard(k)
    ...
    def get_user_stats(self, user_id: int) -> Dict:
        ...
        return {
            ...
            'current_key_index': session['current_key_index'],
            'failed_keys_count': len(session.get('failed_keys', set()))
        }
    ...
    def get_system_stats(self) -> Dict:
        ...
        available_keys = sum(1 for k in GEMINI_API_KEYS if self._is_key_available(k))
        ...
        return {
            ...
            'rotation_enabled': self.key_rotation_enabled
        }
...
# ========== AI GENERATION FUNCTIONS WITH KEY ROTATION ==========
async def generate_with_gemini_advanced(prompt: str, user_id: int, max_retries: int = 3) -> Optional[str]:
    """Усовершенствованная генерация с интеллектуальной ротацией ключей"""
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if "429" in error_str or "quota" in error_str or "resource exhausted" in error_str:
                ...
                logger.info(f"🔄 Ключ {key_index} исчерпал лимит, ротируем...")
                
            elif "403" in error_str or "permission denied" in error_str:
                ai_manager.mark_key_error(key, "403", user_id)
                logger.info(f"🔄 Ключ {key_index} получил 403, ротируем...")
                
            elif "503" in error_str or "unavailable" in error_str:
                ai_manager.rotate_model()
                logger.info(f"🔄 Модель недоступна, меняем на {ai_manager.get_current_model()}")
                
            else:
                logger.error(f"Неизвестная ошибка: {e}")
                ai_manager.mark_key_error(key, "unknown", user_id)
            ...
            if attempt < max_retries:
                wait_time = 1 * attempt
                logger.info(f"🔄 Ротация ключа для user_{user_id}, попытка #{attempt+1} через {wait_time}с")
                await asyncio.sleep(wait_time)
            else:
                logger.error(f"Все {max_retries} попыток исчерпаны для user_{user_id}")
                # Сбрасываем failed_keys для пользователя при полном провале
...
async def init_database():
    ...
    queries = [
        ...
        CREATE TABLE IF NOT EXISTS users (
            ...
            INDEX idx_users_trial (trial_activated, trial_end_date)
        )
        ''',
        ...
        # Таблица подписок на канал для пробного периода
        '''
        CREATE TABLE IF NOT EXISTS channel_subscriptions (
            ...
            INDEX idx_subs_verified (verified),
            INDEX idx_subs_channel (channel_url)
        )
        '''
    ]
...
async def migrate_database():
    ...
    try:
        columns_to_add = [
            ...
            ('users', 'trial_activated', 'BOOLEAN DEFAULT FALSE'),
            ('users', 'trial_start_date', 'TIMESTAMPTZ'),
            ('users', 'trial_end_date', 'TIMESTAMPTZ'),
...
async def get_user_tariff(user_id: int) -> str:
    ...
    user = await execute_query(
        "SELECT tariff, is_admin, tariff_expires_at, trial_activated, trial_end_date FROM users WHERE id = $1", 
        user_id
    )
    ...
    tariff_expires_at = user_data.get('tariff_expires_at')
    if tariff_expires_at:
        if isinstance(tariff_expires_at, str):
            from dateutil import parser
            tariff_expires_at = parser.parse(tariff_expires_at)
        
        if tariff_expires_at < datetime.now(MOSCOW_TZ):
            # Тариф истек, возвращаем к mini
            await execute_query(
                "UPDATE users SET tariff = 'mini', tariff_expires_at = NULL WHERE id = $1",
                user_id
            )
            return 'mini'
    
    # Проверяем пробный период
    if user_data.get('trial_activated'):
        ...
        if trial_end_date:
            ...
            if trial_end_date < datetime.now(MOSCOW_TZ):
                # Пробный период истек, возвращаем к mini
                await execute_query(
                    "UPDATE users SET tariff = 'mini', trial_activated = FALSE WHERE id = $1",
                    user_id
                )
                return 'mini'
            else:
    ...
    return user_data.get('tariff', 'mini')
...
async def get_user_stats(user_id: int) -> Dict:
    ...
    try:
        ...
        user_data = await execute_query(
            "SELECT tariff_expires_at, trial_activated, trial_end_date FROM users WHERE id = $1",
            user_id
        )
        ...
        if user_data:
            ...
            if user_info.get('tariff_expires_at'):
                expires_at = user_info['tariff_expires_at']
                if isinstance(expires_at, str):
                    from dateutil import parser
                    expires_at = parser.parse(expires_at)
                expires_at = expires_at.astimezone(MOSCOW_TZ)
                days_left = (expires_at - datetime.now(MOSCOW_TZ)).days
                tariff_expires_info = f" (осталось {days_left} дней)"
            
            elif user_info.get('trial_activated') and user_info.get('trial_end_date'):
                trial_end = user_info['trial_end_date']
                if isinstance(trial_end, str):
                    from dateutil import parser
                    trial_end = parser.parse(trial_end)
                trial_end = trial_end.astimezone(MOSCOW_TZ)
                days_left = (trial_end - datetime.now(MOSCOW_TZ)).days
                tariff_expires_info = f" (пробный, осталось {days_left} дней)"
        
        return {
            'tariff': tariff_info['name'] + tariff_expires_info,
            ...
            'ai_key_rotation': ai_stats['failed_keys_count']
        }
    except Exception as e:
...
async def get_all_users() -> List[Dict]:
    ...
    return await execute_query('''
        SELECT id, username, first_name, tariff, is_admin, created_at,
               tariff_expires_at, trial_activated, trial_end_date
        FROM users 
...
async def get_user_by_id(user_id: int) -> Optional[Dict]:
    ...
    result = await execute_query(
        "SELECT id, username, first_name, tariff, is_admin, created_at, tariff_expires_at FROM users WHERE id = $1",
        user_id
    )
...
async def update_user_tariff(user_id: int, tariff: str, duration_days: int = 30) -> bool:
    """Обновляет тариф пользователя"""
    try:
        expires_at = datetime.now(MOSCOW_TZ) + timedelta(days=duration_days)
        
        # Если это пробный тариф, устанавливаем даты пробного периода
        if tariff == 'standard_trial':
            trial_end_date = datetime.now(MOSCOW_TZ) + timedelta(days=TARIFFS[tariff].get('trial_days', 3))
            await execute_query('''
                UPDATE users SET 
                tariff = $1, 
...
async def check_channel_subscription(user_id: int) -> Tuple[bool, str]:
    """Проверяет подписку на канал для пробного периода"""
    try:
        # Проверяем, не активировал ли уже пользователь пробный период
        user = await execute_query(
            "SELECT trial_activated, trial_end_date FROM users WHERE id = $1",
            user_id
        )
        
        if user and user[0].get('trial_activated'):
            trial_end_date = user[0].get('trial_end_date')
            if trial_end_date:
                if isinstance(trial_end_date, str):
                    from dateutil import parser
                    trial_end_date = parser.parse(trial_end_date)
                ...
                if trial_end_date > datetime.now(MOSCOW_TZ):
                    days_left = (trial_end_date - datetime.now(MOSCOW_TZ)).days
                    return False, f"🎁 У вас уже активирован пробный период! Осталось {days_left} дней"
        
        # Проверяем, есть ли запись о подписке
        subscription = await execute_query(
            "SELECT verified, trial_granted FROM channel_subscriptions WHERE user_id = $1 AND channel_url = $2",
            user_id, TRIAL_CHANNEL_URL
        )
        
        if subscription:
            sub_info = subscription[0]
            if sub_info.get('trial_granted'):
                return False, "🎁 Вы уже получили пробный период за подписку на канал"
            elif sub_info.get('verified'):
                # Подписка подтверждена, но пробный период не выдан
                success = await update_user_tariff(user_id, 'standard_trial')
                if success:
                    await execute_query(
                        "UPDATE channel_subscriptions SET trial_granted = TRUE, trial_granted_at = NOW() WHERE user_id = $1 AND channel_url = $2",
                        user_id, TRIAL_CHANNEL_URL
                    )
                    return True, "✅ Пробный период на 3 дня успешно активирован! 🎉"
        
        return False, "❌ Подписка на канал не подтверждена"
    except Exception as e:
        logger.error(f"Ошибка проверки подписки: {e}")
        return False, f"❌ Ошибка при проверке подписки: {str(e)}"

async def record_channel_subscription(user_id: int) -> bool:
    """Записывает информацию о подписке на канал"""
    try:
        await execute_query('''
            INSERT INTO channel_subscriptions (user_id, channel_url, subscribed_at)
            VALUES ($1, $2, NOW())
            ON CONFLICT (user_id, channel_url) DO UPDATE 
            SET subscribed_at = NOW(), verified = FALSE, trial_granted = FALSE
        ''', user_id, TRIAL_CHANNEL_URL)
        return True
    except Exception as e:
...
async def verify_channel_subscription(user_id: int) -> bool:
    """Проверяет и подтверждает подписку на канал"""
    try:
        # В реальном боте здесь должна быть проверка через Telegram API
        # Но для простоты мы просто помечаем как подтвержденную
        await execute_query('''
...
def get_main_menu(user_id: int, is_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    if support_available:
        support_row.append(InlineKeyboardButton(text="🆘 Техподдержка", url=support_url))
    else:
        support_row.append(InlineKeyboardButton(text="🆘 Поддержка (альтернатива)", url=support_url))
    
    buttons.append(support_row)
    
    # Кнопка для проблем с поддержкой
    buttons.append([InlineKeyboardButton(text="⚠️ Не работает поддержка?", callback_data="support_problem")])
...
def get_ai_main_menu(user_tariff: str) -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="🔄 Статус ротации ключей", callback_data="rotation_status")],
...
def get_tariffs_keyboard(user_tariff: str = 'mini') -> InlineKeyboardMarkup:
    ...
    for tariff_id, tariff_info in TARIFFS.items():
        ...
        if tariff_id == user_tariff:
            ...
            if price == 0 and tariff_id != 'standard_trial':
                button_text = f"{name} - Бесплатно"
            elif tariff_id == 'standard_trial':
                button_text = f"{name} - Бесплатно за подписку"
            else:
    ...
    buttons.append([InlineKeyboardButton(text="🎁 Получить пробные 3 дня", callback_data="get_trial")])
...
def get_tariff_order_keyboard(tariff_id: str) -> InlineKeyboardMarkup:
    ...
    if tariff_info['price'] == 0:
        if tariff_id == 'standard_trial':
            buttons = [
                [InlineKeyboardButton(text="🎁 Получить 3 дня бесплатно", callback_data="get_trial")],
                [InlineKeyboardButton(text="⬅️ Назад к тарифам", callback_data="tariffs")]
            ]
        else:
            buttons = [
...
def get_admin_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔄 Управление AI ключами", callback_data="admin_ai_keys")],
...
def get_tariff_selection_keyboard() -> InlineKeyboardMarkup:
    ...
    for tariff_id, tariff_info in TARIFFS.items():
        ...
        if price == 0 and tariff_id != 'standard_trial':
            ...
            button_text = f"{name} - Пробный 3 дня"
        else:
...
def get_rotation_status_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура статуса ротации"""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Принудительная ротация", callback_data="force_rotation")],
        [InlineKeyboardButton(text="📊 Статистика ключей", callback_data="key_stats")],
        [InlineKeyboardButton(text="⬅️ Назад в AI меню", callback_data="ai_services")]
    ])

def get_trial_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура для получения пробного периода"""
    return InlineKeyboardMarkup(inline_keyboard=[
...
def get_support_problem_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура для проблем с поддержкой"""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌐 Сайт поддержки", url=RAILWAY_DEPLOY_URL)],
        [InlineKeyboardButton(text="📧 Написать администратору", url=f"https://t.me/{ADMIN_CONTACT.replace('@', '')}")],
        [InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="back_to_main")]
    ])
...
async def cmd_start(message: Message):
    ...
    # Добавляем информацию о ротации ключей
    rotation_info = "• 🔄 Автоматическая ротация ключей AI: ✅ ВКЛЮЧЕНА"
    ...
    welcome_text = (
        ...
        f"{rotation_info}\n"
        f"{support_text}\n\n"
        f"⚠️ Если кнопка поддержки не работает, используйте альтернативный вариант\n\n"
...
async def cmd_help(message: Message):
    ...
    help_text = (
        ...
        "• 🎁 Standard (3 дня) - пробный период за подписку на канал\n\n"
        
        f"🆘 Поддержка: {SUPPORT_URL}\n"
        f"🌐 Альтернативная поддержка: {RAILWAY_DEPLOY_URL}\n"
...
async def show_support_link(callback: CallbackQuery):
    ...
    support_text = (
        ...
        f"🔗 {SUPPORT_URL}\n\n"
        f"🌐 Альтернативная ссылка: {RAILWAY_DEPLOY_URL}\n\n"
...
async def support_problem_handler(callback: CallbackQuery):
    """Обработчик проблемы с поддержкой"""
    problem_text = (
        "⚠️ Проблемы с поддержкой?\n\n"
        "Если кнопка поддержки не работает, вы можете:\n\n"
        "1. 🌐 Перейти на сайт поддержки\n"
        "2. 📧 Написать администратору напрямую\n"
        "3. 🔄 Подождать некоторое время и попробовать снова\n\n"
        "📌 Также вы можете использовать команду /help для получения дополнительной информации."
    )
    
    await callback.message.edit_text(
        problem_text,
        reply_markup=get_support_problem_keyboard()
    )
...
async def start_generation(callback: CallbackQuery, state: FSMContext, word_count: int):
    ...
    preview_text = (
        ...
        f"⏳ Генерирую текст... Использую ротацию ключей (макс. {MAX_403_RETRIES} попытки)"
    )
    ...
    # Индикатор прогресса с информацией о ротации
    progress_msg = await callback.message.answer("🔄 Начинаю генерацию... Ротация ключей активна")
    ...
    if not generated_text:
        ...
        session = ai_manager.get_session(user_id)
        failed_keys_count = len(session.get('failed_keys', set()))
        available_keys = sum(1 for k in GEMINI_API_KEYS if ai_manager._is_key_available(k))
        
        await callback.message.edit_text(
            f"❌ Не удалось сгенерировать текст после {MAX_403_RETRIES} попыток!\n\n"
            f"📊 Статистика ротации:\n"
            f"• Использовано ключей: {session.get('current_attempts', 0)}\n"
            f"• Заблокировано ключей: {failed_keys_count}\n"
            f"• Доступно ключей: {available_keys}/{len(GEMINI_API_KEYS)}\n\n"
            f"📌 Что можно сделать:\n"
    ...
    result_text = (
        ...
        f"• Использовано ключей: {attempts}\n"
...
async def start_generation_for_message(message: Message, data: Dict, word_count: int):
    ...
    # Индикатор прогресса
    progress_msg = await message.answer("🔄 Начинаю генерацию... Ротация ключей активна")
...
async def generate_ideas(callback: CallbackQuery, state: FSMContext):
    ...
    loading_msg = await callback.message.answer("🔄 ИИ генерирует идеи... Использую ротацию ключей")
...
async def show_ai_limits(callback: CallbackQuery):
    ...
    limits_text = (
        ...
        f"🔄 Статистика ротации:\n"
        f"• Доступно ключей: {available_keys}/{len(GEMINI_API_KEYS)}\n"
        f"• Заблокировано: {failed_keys_count}\n"
...
@router.callback_query(F.data == "rotation_status")
async def show_rotation_status(callback: CallbackQuery):
    """Показывает статус ротации ключей"""
    user_id = callback.from_user.id
    ...
    key_stats_text = "🔑 Статистика ключей:\n\n"
    for key_short, stats in system_stats['key_stats'].items():
        status = "✅ Доступен" if not stats['blocked'] else "⛔ Заблокирован"
        key_stats_text += f"{key_short}: {status}\n"
        key_stats_text += f"   Запросы: {stats['requests']}, Успешно: {stats['successful']}, Ошибки: {stats['errors']}\n"
    
    rotation_text = (
        f"🔄 Статус ротации ключей AI\n\n"
        f"📊 Общая статистика:\n"
        f"• Всего ключей: {system_stats['total_keys']}\n"
        f"• Доступно ключей: {system_stats['available_keys']}\n"
        f"• Ротация: {'✅ ВКЛЮЧЕНА' if ai_manager.key_rotation_enabled else '❌ ВЫКЛЮЧЕНА'}\n\n"
        f"📊 Ваша статистика:\n"
        f"• Текущий ключ: #{session.get('current_key_index', 0)}\n"
        f"• Заблокировано ключей: {len(session.get('failed_keys', set()))}\n"
        f"• Всего запросов: {session['total_requests']}\n"
        f"• Последний успешный ключ: {'✅' if session.get('last_successful_key') else '❌'}\n\n"
        f"{key_stats_text}\n"
        f"📍 Ротация автоматически переключает ключи при ошибках 403/429"
    )
    
    await callback.message.edit_text(
        rotation_text,
        reply_markup=get_rotation_status_keyboard()
    )

@router.callback_query(F.data == "force_rotation")
async def force_rotation_handler(callback: CallbackQuery):
    """Принудительная ротация ключа"""
    user_id = callback.from_user.id
    
    # Ротируем ключ
    old_key_index = ai_manager.get_session(user_id).get('current_key_index', 0)
    new_key, new_index = ai_manager.rotate_key(user_id)
    
    await callback.answer(f"🔄 Ключ ротирован: #{old_key_index} → #{new_index}", show_alert=True)
    ...
    rotation_text = (
        f"🔄 Статус ротации ключей AI\n\n"
        f"✅ Принудительная ротация выполнена!\n\n"
        f"📊 Обновленная статистика:\n"
        f"• Текущий ключ: #{new_index}\n"
        f"• Доступно ключей: {system_stats['available_keys']}/{system_stats['total_keys']}\n"
        f"• Заблокировано: {len(session.get('failed_keys', set()))}\n\n"
        f"📍 Новый ключ будет использован для следующего запроса"
    )
...
@router.callback_query(F.data == "key_stats")
async def show_key_stats(callback: CallbackQuery):
    """Показывает детальную статистику по ключам"""
    system_stats = ai_manager.get_system_stats()
    
    stats_text = "🔑 Детальная статистика ключей:\n\n"
    
    for i, key in enumerate(GEMINI_API_KEYS):
        key_short = key[:10] + "..."
        stats = system_stats['key_stats'].get(key_short, {})
        
        if stats:
            status = "✅ Активен" if not stats.get('blocked') else f"⛔ Заблокирован"
            last_used = ""
            if stats.get('last_used'):
                last_used = stats['last_used'].strftime("%H:%M") if isinstance(stats['last_used'], datetime) else stats['last_used']
            
            stats_text += f"{i+1}. {key_short}\n"
            stats_text += f"   Статус: {status}\n"
            stats_text += f"   Запросы: {stats.get('requests', 0)}\n"
            stats_text += f"   Успешно: {stats.get('successful', 0)}\n"
            stats_text += f"   Ошибки: {stats.get('errors', 0)} (403: {stats.get('403_errors', 0)})\n"
            if last_used:
                stats_text += f"   Последний: {last_used}\n"
            stats_text += "\n"
    
    stats_text += f"\n📊 Итого: {system_stats['available_keys']}/{system_stats['total_keys']} ключей доступно"
    
    await callback.message.edit_text(
        stats_text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="rotation_status")]
        ])
    )
...
async def show_ai_examples(callback: CallbackQuery):
    ...
    examples_text = (
        ...
        "🔄 Все тексты сгенерированы с использованием ротации ключей AI"
    )
...
async def show_my_stats(callback: CallbackQuery):
    ...
    stats_text = (
        ...
        f"• 🔄 Заблокировано ключей: {stats.get('ai_key_rotation', 0)}\n\n"
        f"📍 Время по Москве: {datetime.now(MOSCOW_TZ).strftime('%H:%M')}"
    )
    
    await callback.message.edit_text(
        stats_text,
        reply_mup=get_main_menu(user_id, user_id == ADMIN_ID)
    )
...
async def show_tariffs(callback: CallbackQuery):
    ...
    tariffs_text = (
        ...
        "🎁 Standard (3 дня бесплатно):\n"
        "• Все функции Standard\n"
        "• На 3 дня за подписку на канал\n\n"
...
async def tariff_info(callback: CallbackQuery):
    ...
    if tariff_info['price'] == 0:
        if tariff_id == 'standard_trial':
            info_text += "Бесплатно на 3 дня (за подписку на канал)\n\n"
        else:
            info_text += "Бесплатно\n\n"
    else:
    ...
    if tariff_id == current_tariff:
        ...
        info_text += (
            f"🎁 Чтобы получить этот тариф:\n"
            f"1. Подпишитесь на канал: {TRIAL_CHANNEL_URL}\n"
            f"2. Нажмите 'Я подписался'\n"
            f"3. Получите 3 дня бесплатного доступа\n\n"
            f"📋 Ваш ID: {user_id}"
        )
    else:
        info_text += (
...
async def get_trial_handler(callback: CallbackQuery):
    """Обработчик получения пробного периода"""
    user_id = callback.from_user.id
    
    # Проверяем, не активировал ли уже пользователь пробный период
    success, message = await check_channel_subscription(user_id)
    ...
    if success:
        ...
        # Записываем информацию о подписке
        await record_channel_subscription(user_id)
        
        trial_text = (
            f"🎁 Получите 3 дня тарифа Standard бесплатно!\n\n"
            f"Для этого нужно:\n\n"
            f"1. 📢 Подписаться на наш канал: {TRIAL_CHANNEL_URL}\n"
            f"2. ✅ Нажать кнопку 'Я подписался' ниже\n"
            f"3. 🎉 Получить 3 дня бесплатного доступа\n\n"
            f"📋 Ваш ID: {user_id}\n\n"
            f"📍 После подписки вы получите:\n"
            f"• 2 канала, 6 постов в день\n"
            f"• 3 AI-копирайтинга, 30 идей ежедневно\n"
            f"• Все функции тарифа Standard"
        )
...
async def check_subscription_handler(callback: CallbackQuery):
    ...
    # Помечаем подписку как проверенную (в реальном боте здесь должна быть проверка через API)
    await verify_channel_subscription(user_id)
    
    # Проверяем подписку и активируем пробный период
    success, message = await check_channel_subscription(user_id)
    ...
    if success:
        await callback.message.edit_text(
            message,
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🤖 Использовать AI сервисы", callback_data="ai_services")],
                [InlineKeyboardButton(text="⬅️ Назад к тарифам", callback_data="tariffs")]
            ])
        )
    else:
...
async def admin_panel(callback: CallbackQuery):
    ...
    admin_text = (
        ...
        f"• Активных ключей: {system_stats['available_keys']}/{system_stats['total_keys']}\n"
        f"• Ротация ключей: {'✅ ВКЛЮЧЕНА' if ai_manager.key_rotation_enabled else '❌ ВЫКЛЮЧЕНА'}\n"
...
async def admin_stats(callback: CallbackQuery):
    ...
    trial_users = await execute_query("SELECT COUNT(*) as count FROM users WHERE tariff = 'standard_trial'")
    trial_users = trial_users[0]['count'] if trial_users else 0
    ...
    total_subs = await execute_query("SELECT COUNT(*) as count FROM channel_subscriptions")
    total_subs = total_subs[0]['count'] if total_subs else 0
    
    verified_subs = await execute_query("SELECT COUNT(*) as count FROM channel_subscriptions WHERE verified = TRUE")
    verified_subs = verified_subs[0]['count'] if verified_subs else 0
    
    trial_granted = await execute_query("SELECT COUNT(*) as count FROM channel_subscriptions WHERE trial_granted = TRUE")
    trial_granted = trial_granted[0]['count'] if trial_granted else 0
    ...
    stats_text = (
        ...
        f"   • Подтверждено: {verified_subs}\n"
        f"   • Выдано пробных: {trial_granted}\n\n"
        f"🛒 Заказы:\n"
        ...
        if user.get('trial_activated'):
            ...
            if trial_end_date:
                if isinstance(trial_end_date, str):
                    ...
                    trial_end_date = parser.parse(trial_end_date)
                days_left = (trial_end_date - datetime.now(MOSCOW_TZ)).days
                trial_info = f" (пробный, {days_left} дней осталось)"
        
        users_text += f"{i}. ID: {user['id']}\n"
        users_text += f"   Имя: {first_name}\n"
        users_text += f"   Ник: @{username}\n"
        users_text += f"   Тариф: {tariff}{trial_info}\n"
...
async def admin_ai_keys(callback: CallbackQuery):
    """Управление AI ключами"""
    user_id = callback.from_user.id
    ...
    keys_text = (
        ...
        f"• Всего ключей: {system_stats['total_keys']}\n"
        f"• Доступно ключей: {system_stats['available_keys']}\n"
        f"• Заблокировано: {system_stats['total_keys'] - system_stats['available_keys']}\n"
        f"• Ротация: {'✅ ВКЛЮЧЕНА' if ai_manager.key_rotation_enabled else '❌ ВЫКЛЮЧЕНА'}\n\n"
        f"🔑 Список ключей:\n"
    )
    ...
    for i, key in enumerate(GEMINI_API_KEYS):
        key_short = key[:10] + "..."
        stats = system_stats['key_stats'].get(key_short, {})
        status = "✅" if not stats.get('blocked') else "⛔"
        requests = stats.get('requests', 0)
        errors = stats.get('errors', 0)
        
        keys_text += f"{i+1}. {status} {key_short} | Запросы: {requests} | Ошибки: {errors}\n"
    ...
    await callback.message.edit_text(
        keys_text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔄 Включить/выключить ротацию", callback_data="toggle_rotation")],
            [InlineKeyboardButton(text="♻️ Сбросить все блокировки", callback_data="reset_all_blocks")],
            [InlineKeyboardButton(text="📊 Подробная статистика", callback_data="admin_key_stats")],
            [InlineKeyboardButton(text="⬅️ Назад в админку", callback_data="admin_panel")]
        ])
    )

@router.callback_query(F.data == "toggle_rotation")
async def toggle_rotation_handler(callback: CallbackQuery):
    """Включение/выключение ротации"""
    user_id = callback.from_user.id
    ...
    ai_manager.key_rotation_enabled = not ai_manager.key_rotation_enabled
    
    status = "✅ ВКЛЮЧЕНА" if ai_manager.key_rotation_enabled else "❌ ВЫКЛЮЧЕНА"
    
    await callback.answer(f"Ротация ключей {status}", show_alert=True)
    await admin_ai_keys(callback)

@router.callback_query(F.data == "reset_all_blocks")
async def reset_all_blocks_handler(callback: CallbackQuery):
    """Сброс всех блокировок ключей"""
    user_id = callback.from_user.id
    ...
    # Сбрасываем блокировки для всех ключей
    for key in GEMINI_API_KEYS:
        if key in ai_manager.key_stats:
            ai_manager.key_stats[key]['403_errors'] = 0
            ai_manager.key_stats[key]['blocked_until'] = None
            ai_manager.key_stats[key]['consecutive_errors'] = 0
    
    # Сбрасываем failed_keys для всех пользователей
    for user_id, session in ai_manager.sessions.items():
        session['failed_keys'].clear()
        session['consecutive_errors'] = 0
        session['current_attempts'] = 0
...
@router.callback_query(F.data == "admin_key_stats")
async def admin_key_stats_handler(callback: CallbackQuery):
    """Детальная статистика ключей для админа"""
    user_id = callback.from_user.id
    ...
    for i, key in enumerate(GEMINI_API_KEYS):
        ...
        requests = stats.get('requests', 0)
        errors = stats.get('errors', 0)
        errors_403 = stats.get('403_errors', 0)
        successful = stats.get('successful', 0)
        blocked = stats.get('blocked', False)
        
        total_requests += requests
        total_errors += errors
        total_403 += errors_403
        
        status = "✅ Активен" if not blocked else "⛔ Заблокирован"
        success_rate = (successful / requests * 100) if requests > 0 else 0
        
        stats_text += f"{i+1}. {key_short}\n"
        stats_text += f"   Статус: {status}\n"
        stats_text += f"   Запросы: {requests}\n"
        stats_text += f"   Успешно: {successful} ({success_rate:.1f}%)\n"
        stats_text += f"   Ошибки: {errors} (403: {errors_403})\n\n"
    ...
    stats_text += f"• Всего ошибок: {total_errors} (403: {total_403})\n"
    stats_text += f"• Успешность: {overall_success_rate:.1f}%\n"
    stats_text += f"• Доступно ключей: {system_stats['available_keys']}/{system_stats['total_keys']}\n"
    stats_text += f"• Ротация: {'✅ ВКЛЮЧЕНА' if ai_manager.key_rotation_enabled else '❌ ВЫКЛЮЧЕНА'}"
    
    await callback.message.edit_text(
        stats_text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ Назад к управлению ключами", callback_data="admin_ai_keys")]
        ])
    )
...
async def reset_daily_limits_task():
    ...
    try:
        ...
        # Сбрасываем блокировки ключей через 24 часа
        for key in GEMINI_API_KEYS:
            if key in ai_manager.key_stats:
                stats = ai_manager.key_stats[key]
                if stats.get('blocked_until'):
                    if stats['blocked_until'] < datetime.now(MOSCOW_TZ) - timedelta(hours=24):
                        stats['403_errors'] = 0
                        stats['blocked_until'] = None
                        stats['consecutive_errors'] = 0
        
        logger.info("✅ Ежедневные лимиты сброшены, блокировки ключей проверены")
    except Exception as e:
...
async def check_trial_expirations_task():
    """Проверка истечения пробных периодов"""
    try:
        now = datetime.now(MOSCOW_TZ)
        
        # Находим пользователей с истекшим пробным периодом
        expired_trials = await execute_query('''
            SELECT id FROM users 
            WHERE trial_activated = TRUE 
        ...
        for user in expired_trials:
            ...
            try:
                await bot.send_message(
                    user_id,
                    "⏰ Ваш пробный период тарифа Standard истек.\n\n"
                    "Мы вернули вас на бесплатный тариф Mini.\n\n"
                    "Чтобы продолжить использовать все возможности:\n"
                    "1. Выберите тариф Standard или VIP\n"
                    "2. Оформите заказ\n"
                    "3. Получите полный доступ на 30 дней\n\n"
                    "💎 Тарифы: /tariffs"
                )
            except Exception as e:
                logger.error(f"Не удалось уведомить пользователя {user_id} об истечении пробного периода: {e}")
        
        if expired_trials:
            logger.info(f"✅ Проверены пробные периоды, истекло: {len(expired_trials)}")
    except Exception as e:
...
async def on_startup():
    ...
    logger.info(f"🔄 Ротация ключей: {'ВКЛЮЧЕНА' if KEY_ROTATION_ENABLED else 'ВЫКЛЮЧЕНА'}")
    logger.info(f"👑 Admin ID: {ADMIN_ID}")
    logger.info(f"🆘 Поддержка: {SUPPORT_BOT_USERNAME or SUPPORT_URL}")
    logger.info(f"🎁 Канал для пробного периода: {TRIAL_CHANNEL_URL}")
    logger.info("=" * 60)
    ...
    try:
        ...
        # Проверка пробных периодов раз в день
        scheduler.add_job(
            check_trial_expirations_task,
            trigger='cron',
            hour=2,
            minute=0,
            timezone=MOSCOW_TZ,
        ...
        if ADMIN_ID:
            try:
                ...
                await bot.send_message(
                    ...
                    f"🔄 Ротация ключей: {'✅ ВКЛЮЧЕНА' if KEY_ROTATION_ENABLED else '❌ ВЫКЛЮЧЕНА'}\n"
                    f"🎁 Канал для пробного периода: {TRIAL_CHANNEL_URL}\n"
                    f"🆘 Поддержка: {SUPPORT_URL}\n"
                    f"🌐 Альтернативная поддержка: {RAILWAY_DEPLOY_URL}\n"
                    f"🕐 Время: {datetime.now(MOSCOW_TZ).strftime('%d.%m.%Y %H:%M:%S')}"
                )
        ...
        logger.info("=" * 60)
        logger.info("🎉 БОТ УСПЕШНО ЗАПУЩЕН С AI СЕРВИСАМИ И РОТАЦИЕЙ КЛЮЧЕЙ!")
        logger.info("=" * 60)
        return True
