...
КОЛИЧЕСТВОР СЛОВ: {word_count} слов
...
async def save_scheduled_post(user_id: int, channel_id: int, post_data: Dict, scheduled_time: datetime) -> Optional[int]:
    ...
    try:
        ...
        # Правильная обработка post_data - проверяем тип
        if isinstance(post_data, dict):
            ...
            # Если пришел не dict, устанавливаем значения по умолчанию
            message_type = 'text'
            message_text = str(post_data) if post_data else ''
            media_file_id = None
            media_caption = None
...
async def process_content(message: Message, state: FSMContext):
    ...
    if message.content_type == ContentType.TEXT:
        ...
        photo_size = max(message.photo, key=lambda p: p.file_size)
        post_data = {
            'message_type': 'photo',
            'media_file_id': photo_size.file_id,
...
async def send_scheduled_post(channel_id: int, post_data: Dict, post_id: int):
    ...
    try:
        ...
        # Пытаемся получить пользователя, который запланировал пост
        post_info = await execute_query(
            "SELECT user_id FROM scheduled_posts WHERE id = $1",
            post_id
        )
