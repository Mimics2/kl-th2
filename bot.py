...
КОЛИЧЕСТВОВ СЛОВ: {word_count} слов
...
async def confirm_post(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.info(f"✅ Пост {post_id} запланирован на {scheduled_datetime.strftime('%d.%m.%Y %H:%M')} для канала {channel_id}")
...
# ... остальные админ-хендлеры остаются без изменений ...
    ...
    try:
        ...
        for post in posts:
            try:
                ...
                # Конвертируем в московское время для планировщика
                scheduled_moscow = scheduled_time.astimezone(MOSCOW_TZ)
                
                # Рассчитываем время до публикации
                now = datetime.now(MOSCOW_TZ)
                time_until = scheduled_moscow - now
                
                # Добавляем задачу только если время в будущем
                if time_until.total_seconds() > 0:
                    scheduler.add_job(
                        send_scheduled_post_job,
                        trigger='date',
                        run_date=scheduled_moscow,
                        args=[post['id']],
                        id=f"post_{post['id']}",
                        replace_existing=True,
                    ...
                    restored += 1
                    logger.info(f"✅ Восстановлен пост {post['id']} на {scheduled_moscow.strftime('%d.%m.%Y %H:%M')} МСК")
        ...
        # Отправляем пропущенные посты (не старше 24 часов)
