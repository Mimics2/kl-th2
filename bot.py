...
async def init_database():
    ...
    queries = [
        ...
        # Таблица запланированных постов (ИСПРАВЛЕННАЯ)
        '''
...
# ========== ИСПРАВЛЕННЫЕ ФУНКЦИИ ПУБЛИКАТОРА ==========
async def save_scheduled_post(user_id: int, channel_id: int, post_data: Dict, scheduled_time: datetime) -> Optional[int]:
    ...
    try:
        ...
        if post_id:
            logger.info(f"✅ Пост сохранен в БД с ID: {post_id} на время: {scheduled_time.strftime('%d.%m.%Y %H:%M')} МСК")
...
async def schedule_post_in_scheduler(post_id: int, scheduled_time: datetime):
    """Добавляет пост в планировщик (ИСПРАВЛЕННАЯ ВЕРСИЯ)"""
    try:
        if scheduled_time.tzinfo is None:
            scheduled_time = MOSCOW_TZ.localize(scheduled_time)
        ...
        if scheduled_time <= now:
            ...
            await send_scheduled_post(post_id)
            return False
        ...
        logger.info(f"✅ Пост {post_id} запланирован на {scheduled_time.strftime('%d.%m.%Y %H:%M')} МСК (через {time_until:.1f} часов)")
        return True
...
async def send_scheduled_post(post_id: int):
    """Отправляет запланированный пост (ИСПРАВЛЕННАЯ ВЕРСИЯ)"""
    try:
        ...
        if retry_count >= 3:
            ...
            await execute_query('''
                ...
                SET error_message = $1 
                WHERE id = $2
            ''', "Превышен лимит попыток отправки", post_id)
            return
        ...
        try:
            ...
            await execute_query('''
                ...
                WHERE id = $3
            ''', error_msg, new_retry_count, post_id)
...
async def confirm_post(callback: CallbackQuery, state: FSMContext):
    """Подтверждение публикации (ИСПРАВЛЕННАЯ ВЕРСИЯ)"""
    user_id = callback.from_user.id
    data = await state.get_data()
    ...
    scheduled_success = await schedule_post_in_scheduler(post_id, scheduled_datetime)
        ...
        for post in posts:
            try:
                ...
                if scheduled_time <= now:
                    ...
                    asyncio.create_task(send_scheduled_post(post_id))
                else:
...
async def on_startup():
    """Запуск бота"""
    logger.info("=" * 60)
    logger.info(f"🚀 ЗАПУСК БОТА KOLES-TECH v3.0 (ИСПРАВЛЕННЫЙ ПУБЛИКАТОР)")
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("🎉 БОТ УСПЕШНО ЗАПУЩЕН С ИСПРАВЛЕННЫМ ПУБЛИКАТОРОМ!")
        logger.info("=" * 60)
        return True
